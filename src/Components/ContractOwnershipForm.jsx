...
const SUBMIT_CONTRACT_URL =
  "https://hooks.zapier.com/hooks/catch/9914807/3j9oqgz/";
const SUBMIT_IMPLEMENTATION_URL =
  "https://hooks.zapier.com/hooks/catch/9914807/3j9bz1i/";
        ...
        let interval = setInterval(async () => {
          try {
            const {
              ...
              `${CHECK_PROXY_URL}/${contractAddress}`,
              FETCH_CONFIG,
            );
    ...
    try {
      ...
      if (provider === "SmartContractOwnership") {
        ...
        localStorage.setItem(
          `smartContractOwnership:${userAddress}`,
    ...
    (async () => {
      ...
      try {
        const {
          ...
          `${VERIFY_MESSAGE_SIGNATURE_URL}/${ownerAddress}/verify`,
          {
            signature: signedMessageData,
            message: messageToSign,
          },
  ...
  return (
    <form style={{ width: 300 }} onSubmit={onSubmit}>
      ...
      <p className="instructions">
        Enter your dapp smart contract address below to start the verification
        process
      </p>
      ...
      <input
        ...
        onChange={({ target: { value } }) =>
          onChangeInput("contractAddress", value)
        }
      />
      <br />
      {isProxy && (
        <>
          <label className="result success">
      ...
      <div className={`${[null, false].includes(isProxy) && "hidden"}`}>
        ...
        <input
          ...
          variant="outlined"
          onChange={({ target: { value } }) =>
            onChangeInput("implementationAddress", value)
          }
        />
        <br />
        {ownerAddress && (
      ...
      {ownerAddress && !signedMessageData && (
        <>
          ...
          <label className="result" style={{ float: "left" }}>
            Please sign data with your wallet
          </label>
        </>
      )}
      ...
      {isOwnerVerified !== null &&
        (isOwnerVerified ? (
          <label className="result success" style={{ float: "left" }}>
            ✓ Ownership verified and {xp} points awarded! You can now close this
            window.
          </label>
        ) : (
          <label className="result error" style={{ float: "left" }}>
            x Ownership verification failed, please try signing from a different
            wallet address
          </label>
        ))}
    </form>
