...
const CHECK_PROXY_URL =
  "https://f3ae-109-255-0-100.ngrok-free.app/v1/zapier/proxycontract";
const VERIFY_OWNERSHIP_URL =
  "https://f3ae-109-255-0-100.ngrok-free.app/v1/zapier/contractowner";
  ...
  const submitAuditProof = async () => {
    ...
    try {
      ...
      const myAttestation = myAttestations.find((a) => {
        ...
        return (
          a.decodedPayload[0].byteCodeHash?.toLowerCase() === hash.toLowerCase()
        );
      });
      const { auditorSignature, byteCodeHash } =
        myAttestation.decodedPayload[0];
  ...
  return (
    <form style={{ width: 300 }} onSubmit={onSubmit}>
      ...
      <input
        ...
        variant="outlined"
        onChange={({ target: { value } }) =>
          onChangeInput("auditorAddress", value)
        }
      />
      <button
        className="small-btn"
